...
class DashboardApi {
  ...
  async fetchTotalViews(): Promise<any> {
    try {
      const response = await this.axiosInstance.get('/admin/dashboard-total-orders');
  ...
  async fetchTotalRevenue(period:string): Promise<any> {
    try {
      const response = await this.axiosInstance.get(`/admin/dashboard-total-revenue?period=${period}`);
  ...
  async fetchTotalProducts(): Promise<any> {
    try {
      const response = await this.axiosInstance.get('/admin/dashboard-total-products');
      return response.data; // Ensure your backend sends { total, rate }
    } catch (error) {
